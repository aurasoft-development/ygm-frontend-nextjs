...
function Pagination({ totalPages, handlePageChange, pageName, currentPage }) { 
  const router = useRouter();
  function handlePageChange(page) {
    router.push(`/${pageName}?page=${page}`)
  }
  return (
      ...
      <ul className="pagination">
        ...
        {Array.from({ length: totalPages }).map((_, i) => (
          <Link key={i} href={`/${pageName}?page=${i + 1}`}>
            <li className={`page-item ${Number(currentPage) == i + 1 ? "active" : ""}`}><a className="page-link" onClick={() => handlePageChange(i + 1)}>{i + 1}</a></li>
          </Link>
        ))}
