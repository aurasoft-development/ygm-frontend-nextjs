...
import { getItem, setItem } from '../../utils/utilsFunctions';
const UpdateDetails = (props) => {
    const dispatch = useDispatch();
    const { initDetails } = props;
    const router = useRouter()
    const jobData = router.query.id
    const [file, setFile] = useState("");
    const [fileError, setFileError] = useState("")
    const refFile = useRef()
    const data = JSON.parse(getItem("GYC_login_user_data"));
    const { student_name, phone, token } = data;
    const initialValues = {
        student_name,
        student_phone: phone,
        token
    ...
    const validate = (values) => {
        ...
        if (!values.student_phone) {
            errors.student_phone = "Phone number is required";
        } else if (!validator.isMobilePhone(values.student_phone)) {
            errors.student_phone = "Please enter valid phone number";
        }
        return errors;
    }
    ...
    useEffect(() => {
        const PostData = async () => {
            ...
            dataArray.append("student_phone", formValues.student_phone);
            dataArray.append("profile_pic", file);
            dataArray.append("token", formValues.token);
           
            try {
                const url = "https://getyourcollege.in/gyc_admin/api/update_student_details.php"
            ...
            <Container center>
                <div className=' '>
                    <form onSubmit={handleSubmitA} noValidate data-aos="fade-right" >
                        {/* <h3 className='Form_main_heading'>Update Your Details</h3> */}
                        ...
                        <div className=' mx-auto'> <div className="input-field">
                            ...
                            <input type="number" id='student_phone'
                                ...
                                onChange={handleChange}
                                className={formErrors.student_phone && "t-error"}
                            />
                            {formErrors.student_phone && (
                                <div className='w-100'>
                                    <small className='error'>{formErrors.student_phone}</small>
                                </div>
                            )}
                        </div></div>
