...
function Course({ courseData, query }) {
    ...
    const handleCompare = (e, collegeId, collegeName) => {
        if (e.target.checked && selectedCollege && selectedCollege.length < 3) {
    ...
    useEffect(() => {
        selectedCollege.length === 3 && setShowCompareModal(true);
    }, [selectedCollege])
    ...
    return (
        <>
            <section className='college-list' data-aos="zoom-in-right" data-aos-easing="ease-in-back" data-aos-delay="500" data-aos-offset="0"
            >
                <div className="container" data-aos="zoom-in" data-aos-easing="ease-in-back" data-aos-delay="500" data-aos-offset="0"
                >
                    ...
                    <div className="row mx-auto justify-content-around">
                        {collegeList?.length !== 0 && isBrowser()
                                            ...
                                            <div className="college-details bg-white">
                                                ...
                                                <div className='card-content'>
                                                    ...
                                                    <div className='redirection pb-3'>
                                                        ...
                                                        <Link className="btn btn-white btn-animate mx-auto" href={`/Institutions/collegeform/${college.college_slug}?course=${query.course}&type=${college.college_type}`}>Apply Now</Link>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            : (query.type === '3' ? (
                                <div className='text-danger text-center p-3'>No coaching/ institutions available for this {query.course} course.</div>
                            ) : (
                                <div className='text-danger text-center p-3'>No colleges/ universities available for this {query.course} course.</div>
                            )
                            )
                        }
                    </div>
